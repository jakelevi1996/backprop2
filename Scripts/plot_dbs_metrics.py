"""
Script to plot DBS metrics, when training a neural network on sinusoidal data
using gradient descent with a line-search, using the full training set as a
batch for each update.
...
TODO: make a subdirectory of the results directory, and plot the predictions for
each model

Below are some examples for calling this script:

    python Scripts\plot_dbs_metrics.py 1 1 100 1 10 0 0.02 3

    python Scripts\plot_dbs_metrics.py 1 1 1000 1 10 0 0.02 3

    python Scripts\plot_dbs_metrics.py 2 3 2500 20 20,20 0 4 3

    python Scripts\plot_dbs_metrics.py 2 3 1000 20 20,20 0 4 3

    python Scripts\plot_dbs_metrics.py 2 3 500 20 20,20 0 4 3

    python Scripts\plot_dbs_metrics.py 2 3 200 20 20,20 0 4 3

    python Scripts\plot_dbs_metrics.py 2 3 50 20 20,20 0 4 3

    python Scripts\plot_dbs_metrics.py 2 3 5 20 20,20 0 4 3

To get help information for the available arguments, use the following command:

    python Scripts\plot_dbs_metrics.py -h
...
def main(
    ...
    -   n_repeats: positive integer number of repeats to perform of each
        experiment
    -   batch_size: integer batch size to use for optimisation
    """
    ...
    for i in range(n_repeats):
        ...
        # Calculate gradient vector once for initial DBS metrics
        ...
        result = optimisers.gradient_descent(
            ...
            batch_getter=optimisers.batch.FullTrainingSet(sin_data)
        )
    ...
    plot_name_suffix += ", %i data points"          % n_train
    # plot_name_suffix += ", batch_size = %.3f"       % batch_size
    ...
    for attr_name in ["dbs_metric", "global_dbs_metric"]:
